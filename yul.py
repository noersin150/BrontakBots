...
sm = LineClient()
sm.log("Auth Token : " + str(sm.authToken))
channel12 = LineChannel(sm)
sm.log("Channel Access Token : " + str(channel12.channelAccessToken))
print("---LOGIN SUCCES BOSS- SELFBOT SOLO")
poll = LinePoll(cl)
call = cl
creator = ["u6c47c26607741a8c12a531fdb340b09a"]
owner = ["u6c47c26607741a8c12a531fdb340b09a"]
admin = ["u6c47c26607741a8c12a531fdb340b09a"]
staff = ["u6c47c26607741a8c12a531fdb340b09a"]
...
Gmid = sm.getProfile().mid
KAC = [cl,ki,kk,kc,kb,kd,sw,sm]
ABC = [ki,kk,kc,kb,kd,sw,sm]
Bots = [mid,Amid,Bmid,Cmid,Dmid,Emid,Zmid,Gmid]
Ghost = [sw,sm]
Dpk = admin + staff
...
def help1():
    key = Setmain["keyCommand"]
    key = key.title()
    helpMessage1 = "🔑❂͜͡➣ " + key + " [BRONTAK_BOTS]\n" + \
                  "🔑❂͜͡➣ " + key + "Tagall/Nah\n" + \
                  "🔑❂͜͡➣ " + key + "Ginfo\n" + \
                  "🔑❂͜͡➣ " + key + "Open\n" + \
                  "🔑❂͜͡➣ " + key + "Close\n" + \
                ...
                try:
                    if cl.getGroup(op.param1).preventedJoinByTicket == False:
                        if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                            ...
                            sm.acceptGroupInvitationByTicket(op.param1,Ticket)
                            sw.kickoutFromGroup(op.param1,[op.param2])
                            sm.kickoutFromGroup(op.param1,[op.param2])
                            sw.leaveGroup(op.param1)
                            sm.leaveGroup(op.param1)
                            cl.updateGroup(X)
                except:
                    try:
            ...
            if mid in op.param3:
                if wait["autoLeave"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        cl.sendMessage(op.param1,"See you Guyss\n Group " +str(ginfo.name))
                        cl.leaveGroup(op.param1)
                    else:
                        cl.acceptGroupInvitation(op.param1)
                        ginfo = cl.getGroup(op.param1)
                        cl.sendMessage(op.param1,"Assalamualaikum " + str(ginfo.name))
            ...
            if Amid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        ki.sendMessage(op.param1,"See You Guyss\n Group " +str(ginfo.name))
                        ki.leaveGroup(op.param1)
                    else:
                        ki.acceptGroupInvitation(op.param1)
                        ginfo = ki.getGroup(op.param1)
                        ki.sendMessage(op.param1,"Assalamualaikum " + str(ginfo.name))
            if Bmid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        kk.acceptGroupInvitation(op.param1)
                        ginfo = kk.getGroup(op.param1)
                        ki.sendMessage(op.param1,"See You Guss\n Group " +str(ginfo.name))
                        kk.leaveGroup(op.param1)
                    else:
                        kk.acceptGroupInvitation(op.param1)
                        ginfo = kk.getGroup(op.param1)
                        kk.sendMessage(op.param1,"Assalamualaikum " + str(ginfo.name))
            if Cmid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        kc.acceptGroupInvitation(op.param1)
                        ginfo = kc.getGroup(op.param1)
                        kc.sendMessage(op.param1,"See You guys\n Group " +str(ginfo.name))
                        kc.leaveGroup(op.param1)
                    else:
                        kc.acceptGroupInvitation(op.param1)
                        ginfo = kc.getGroup(op.param1)
                        kc.sendMessage(op.param1,"Assalamualaikum " + str(ginfo.name))
                ...
                if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                    try:
                        ...
                        for target in gMembMids:
                            ...
                            cl.sendMessage(op.param1,"mamam tuh..😂😂😂😂😜😜")
                            wait["blacklist"][op.param2] = True
                    except:
                        try:
            ...
            try:
                if op.param1 in ghost:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        sw.kickoutFromGroup(op.param1,[op.param2])
                        sm.kickoutFromGroup(op.param1,[op.param2])
                        sm.sendMessage(op.param1,"mamam tuh..😂😂😂😂😜😜")
                        sw.leaveGroup(op.param1)
                        sm.leaveGroup(op.param1)
                        X = cl.getGroup(op.param1)
                        X.preventedJoinByTicket = True
                ...
                if op.param1 in protectantijs:
                  if op.param3 in mid:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        sm.acceptGroupInvitation(op.param1)
                        G = sw.getGroup(op.param1)
                        G = sm.getGroup(op.param1)
                        G.prevenARoinByTicket = False
                        sw.updateGroup(G)
                        Ticket = sw.reissueGroupTicket(op.param1)
                        Ticket = sm.reissueGroupTicket(op.param1)
                        cl.acceptGroupInvitationByTicket(op.param1,Ticket)
                        sw.kickoutFromGroup(op.param1,[op.param2])
                        sm.kickoutFromGroup(op.param1,[op.param2])
                        sm.sendMessage(op.param1,"MAMAM TUUHH..😂😂😂😂😜😜")
                        G.prevenARoinByTicket = True
                        sw.updateGroup(G)
                        wait["blacklist"][op.param2] = True
                        sw.leaveGroup(op.param1)
                        sm.leaveGroup(op.param1)
                        cl.inviteIntoGroup(op.param1,[Zmid,Gmid])
            ...
            if op.param1 in protectcancel:
                if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                    ...
                    try:
                        if op.param3 not in wait["blacklist"]:
                            ki.kickoutFromGroup(op.param1,[op.param2])
                            ki.sendMessage(op.param1,"Heh Maen cancel cipok ni 🙍")
                    except:
                        try:
                            if op.param3 not in wait["blacklist"]:
                                kk.kickoutFromGroup(op.param1,[op.param2])
                                kk.sendMessage(op.param1,"Heh Maen cancel cipok ni 🙍")
                        except:
                            try:
                                if op.param3 not in wait["blacklist"]:
                                    kc.kickoutFromGroup(op.param1,[op.param2])
           ...
           if wait["selfbot"] == True:
               ...
               if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                   ...
                   image = "http://dl.profile.line-cdn.net/" + contact.picture
...
#KICKALL
                        elif "!brontak" in msg.text:
                          if msg._from in admin:
                           if msg.toType == 2:
                              print("ok")
                              _name = msg.text.replace("!brontak","")
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                              try:
                                  tz = pytz.timezone("Asia/Jakarta")
                                  timeNow = datetime.now(tz=tz)
                                  cl.sendMessage(msg.to, "「 Status Razia 」\nBerhasil diaktifkan\n\n• Jam [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]"+"\n• Tanggal : "+ datetime.strftime(timeNow,'%Y-%m-%d'))
...
">>>BYE..BYE>><\nJIKA DENDAM CARI KAMI \n<<<<<<<<<>>\nhttp://line.me/ti/p/~gerhanaselatan\nhttp://line.me/ti/p/~cyber01")
                        ...
                        elif cmd == "Brontak" or cmd == "sdbotku":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               start = time.time()
                               cl.sendMessage(msg.to, "[BRONTAK_BOST].")
                               elapsed_time = time.time() - start
...
">>>BYE BYE><\nMAU DENDAM CARI KAMI\n<<<<<<<<<>>\nhttp://line.me/ti/p/~gerhanaselatan\nhttp://line.me/ti/p/~cyber01")
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                                ...
                                sG.sendMessage(msg.to,"Send your images.....")
                        ...
                        elif cmd.startswith("ghost2cname: "):
                          if msg._from in admin:
                            separate = msg.text.split(" ")
                            string = msg.text.replace(separate[0] + " ","")
                            if len(string) <= 10000000000:
                                ...
                                sm.sendMessage(msg.to,"Nama diganti jadi " + string + "")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                try:
                                    ginfo = cl.getGroup(msg.to)
                                    cl.inviteIntoGroup(msg.to, [Zmid,Gmid])
                                    cl.sendMessage(msg.to,"Grup"+str(ginfo.name)+"Bidik di luar boss")
                                except:
                                    pass
...
">>>BYEE-BYEE<..\nLU DENDAM GUE TUNGGU\n<<<<<<<<<>>\nhttp://line.me/ti/p/~gerhanaselatan\nhttp://line.me/ti/p/~cyber01")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                ki.sendMessage(msg.to, "SIAP PERINTAH BOSS "+str(G.name))
                                kk.acceptGroupInvitationByTicket(msg.to,Ticket)
                                kk.sendMessage(msg.to, "SIAP PERINTAH BOSS "+str(G.name))
                                kc.acceptGroupInvitationByTicket(msg.to,Ticket)
                                kc.sendMessage(msg.to, "SIAP PERINTAH BOSS "+str(G.name))
                                kb.acceptGroupInvitationByTicket(msg.to,Ticket)
                                kb.sendMessage(msg.to, "SIAP PERINTAH BOSS "+str(G.name))
                                kd.acceptGroupInvitationByTicket(msg.to,Ticket)
                                kd.sendMessage(msg.to, "SIAP PERINTAH BOSS "+str(G.name))
                                G = cl.getGroup(msg.to)
                                G.preventedJoinByTicket = True
                                cl.updateGroup(G)
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                                ...
                                sm.acceptGroupInvitationByTicket(msg.to,Ticket)
                                sw.sendMessage(msg.to, "Ghost masuk "+str(G.name))
                                G = sw.getGroup(msg.to)
                                G = sm.getGroup(msg.to)
                                G.preventedJoinByTicket = True
                                sw.updateGroup(G)
                               ...
                               for target in targets:
                                   if target not in Bots:
                                       try:
                                           ...
                                           sm.kickoutFromGroup(msg.to, [target])
                                           sw.leaveGroup(msg.to)
                                           sm.leaveGroup(msg.to)
                                           X = cl.getGroup(msg.to)
                                           X.preventedJoinByTicket = True
...
#===========KICKOUT============#
                        ...
                        elif cmd == "guys" or cmd == "sdbot":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               start = time.time()
                               cl.sendMessage(msg.to, "[BRONTAK_BOTS].")
                               elapsed_time = time.time() - start
                               ...
                               ki.sendMessage(msg.to, "╚JAMIN AMAN╗\n╚BOSS☆╮╗")
...
#===========BOT UPDATE============#
                        ...
                        elif 'Tuyul ' in msg.text:
                           if msg._from in admin:
                              spl = msg.text.replace('Tuyul ','')
                              if spl == 'on':
                                  if msg.to in ghost:
                                       msgs = "[Done ] \nTuyul\nsudah aktif"
                                  else:
                                       ghost.append(msg.to)
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "Tuyul Diaktifkan\nDi Group : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "╭━━━╮\n┃╭━╮┃\n┃┃╱┃┣╮╭┳━━┳━╮\n┃╰━╯┃╰╯┃╭╮┃╭╮╮\n┃╭━╮┃┃┃┃╭╮┃┃┃┃\n╰╯╱╰┻┻┻┻╯╰┻╯╰╯ \n" + msgs)
                              elif spl == 'off':
                                    if msg.to in ghost:
                                         ghost.remove(msg.to)
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "Tuyul Dinonaktifkan\nDi Group : " +str(ginfo.name)
                                    else:
                                         msgs = "[Done]\nTuyul\nSudah Tidak Aktif"
                                    cl.sendMessage(msg.to, "╔═══╗\n╚╗╔╗║\n─║║║╠══╦═╗╔══╗\n─║║║║╔╗║╔╗╣║═╣\n╔╝╚╝║╚╝║║║║║═╣\n╚═══╩══╩╝╚╩══╝\n" + msgs)
#===========ADMIN ADD============#
                            ...
                            if msg._from in admin:
                              if wait["blacklist"] == {}:
                                ...
                                for m_id in wait["blacklist"]:
                                    ...
                                    ma += str(a) + ". " +cl.getContact(m_id).displayName + "\n"
                                cl.sendMessage(msg.to,"BRONTAK  BOTS Blacklist User\n\n"+ma+"\nTotal「%s」Blacklist User" %(str(len(wait["blacklist"]))))
                            ...
                            if msg._from in admin:
                              if wait["Talkblacklist"] == {}:
                                ...
                                for m_id in wait["Talkblacklist"]:
                                    ...
                                    ma += str(a) + ". " +cl.getContact(m_id).displayName + "\n"
                                cl.sendMessage(msg.to,"BRONTAK BOTS Talkban User\n\n"+ma+"\nTotal「%s」Talkban User" %(str(len(wait["Talkblacklist"]))))

                        elif cmd == "teror" or text.lower() == 'blc':
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                              if wait["blacklist"] == {}:
...
#===========Protection============#
                        ...
                        elif 'Pintu ' in msg.text:
                           if msg._from in admin:
                              spl = msg.text.replace('Pintu ','')
                              if spl == 'on':
                                  if msg.to in protectjoin:
                                       msgs = "Pintu dikunci Boss..😎😎😎"
                                  else:
                                       protectjoin.append(msg.to)
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "Status : [ Terkuci ]\nDi Group : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「 Status Pintu terkunci boss」\n" + msgs)
                              elif spl == 'off':
                                    if msg.to in protectjoin:
                                         protectjoin.remove(msg.to)
                                         ginfo = cl.getGroup(msg.to)
                                         ...
                                         msgs = "Pintu Terbuka.silahkan masuk..😄😄😄"
                                    cl.sendMessage(msg.to, "「 silahkan masuk 」\n" + msgs)
                        ...
                        elif 'Pall ' in msg.text:
                           if msg._from in admin:
                              spl = msg.text.replace('Pall ','')
                              if spl == 'on':
                                  if msg.to in protectqr:
                                       msgs = ""
                                  else:
